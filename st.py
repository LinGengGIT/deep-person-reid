...
# import models
from util.losses import CrossEntropyLabelSmooth, TripletLoss,OIMLoss
...
from models.SANet import stattention
...
parser.add_argument('--test-batch', default=8, type=int, help="test batch size (number of tracklets)")
...
parser.add_argument('--spanum', default=3, type=int, help='spatial number')
...
def main():
    ...
    trainloader = DataLoader(
        VideoDataset(dataset.train, seq_len=args.seq_len, sample='satten', transform=transform_train), shuffle=True,
    ...
    model = stattention(dataset.num_train_pids, loss={'xent', 'htri'}, seq_len=args.seq_len)
    print("Model size: {:.5f}M".format(sum(p.numel() for p in model.parameters())/1000000.0))

    criterion_xent = CrossEntropyLabelSmooth(num_classes=dataset.num_train_pids, use_gpu=use_gpu)
    criterion_htri = TripletLoss(margin=args.margin)
    criterion_oim=OIMLoss(num_features=args.train_batch, num_classes=args.spanum*128)

    param_groups = [{'params': model.base_model.parameters(), 'lr_mult': 1.0},
                    {'params': model.feature_model.parameters(), 'lr_mult': 10.0},
                    {'params': model.sattention.parameters(), 'lr_mult': 10.0},
    ...
    if args.evaluate:
        print("Evaluate only")
        test(model, queryloader, galleryloader, args.pool, use_gpu,criterion_xent, criterion_htri)
        return
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        train(epoch, model, criterion_xent, criterion_htri, criterion_oim,optimizer, trainloader, use_gpu,args.pool)
        ...
        if args.eval_step > 0 and (epoch+1) % args.eval_step == 0 or (epoch+1) == args.max_epoch:
            print("==> Test")
            rank1 = test(model, queryloader, galleryloader, args.pool, use_gpu,criterion_xent, criterion_htri)
...
def train(epoch, model, criterion_xent, criterion_htri,criterion_oim, optimizer, trainloader, use_gpu,pool):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses=AverageMeter()
    losses_seq = AverageMeter()
    loss_xent = AverageMeter()
    loss_htri = AverageMeter()
    model.train()
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        features, scores,reg = model(imgs)
        features = features.view(-1, args.seq_len, args.spanum, features.size(-1))
        scores = scores.view(-1, args.seq_len, args.spanum)
        scores = F.softmax(scores, dim=1)

        reg = torch.sqrt(reg)
        reg = torch.bmm(reg, reg.transpose(1,2))
        reg = reg-torch.eye(reg.size(1)).expand_as(reg).cuda()
        reg = torch.pow(reg, 2)
        ...
        if pool == 'avg':
            features = features.view(batch_size, args.seq_len, args.spanum, -1)
        ...
        if pool =='qan':
            ...
            features = features.view(batch_size, args.seq_len, args.spanum,-1)
            scores = scores.view(batch_size, args.seq_len, args.spanum,-1).expand(batch_size, args.seq_len, args.spanum,features.size(-1))
        ...
        if args.htri_only:
            ...
            #loss_seq,_=criterion_oim(features, labels)
        ...
        loss_xent.update(xent_loss.item(), labels.size(0))
        loss_htri.update(htri_loss.item(), labels.size(0))
        ...
        if (batch_idx+1) % args.print_freq == 0:
            ...
            print('Epoch: [{0}/{1}][{2}/{3}]\t'
                  ...
                  'XentLoss {xent_loss.val:.6f} ({xent_loss.avg:.6f})\t'
                  'TriLoss {htri_loss.val:.6f} ({htri_loss.avg:.6f})'.format(
                   epoch+1, args.max_epoch, batch_idx+1, len(trainloader), batch_time=batch_time,
                   data_time=data_time,  seq_loss=losses_seq, loss=losses,xent_loss=loss_xent,htri_loss=loss_htri))


def test(model, queryloader, galleryloader, pool, use_gpu, criterion_xent, criterion_htri,ranks=[1, 5, 10, 20]):
    model.eval()
    # losses=AverageMeter()
    # losses_seq = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (imgs, pids, camids) in enumerate(queryloader):
            ...
            features = features.view(b, s,args.spanum, -1)
            ...
            if pool == 'avg':
                ...
                scores = scores.view(b, s,args.spanum, -1).expand(b, s, args.spanum,features.size(-1))
                features = (features * scores).sum(1)
                scores = scores.sum(1)
                features = features / scores
        ...
        # losses_seq.update(loss_seq.item(), labels.size(0))
        # losses.update(loss.item(), labels.size(0))
        # tri_loss.update(htri_loss.item(),labels.size(0))###
        ...
        #         'Loss {loss.val:.6f} ({loss.avg:.6f})\t'
        #         'TriLoss {htri_loss.val:.6f} ({htri_loss.avg:.6f})'.format(
        #         args.max_epoch, batch_idx + 1, len(queryloader), batch_time=batch_time,
        #     data_time=data_time, seq_loss=losses_seq, loss=losses,htri_loss=tri_loss))
        ...
        for batch_idx, (imgs, pids, camids) in enumerate(galleryloader):
            ...
            if pool == 'avg':
                ...
                scores = scores.sum(1)
                features = features / scores
                features = features.view(-1, args.spanum*128)
            else:
                features = features.view(b, s, -1)
                features, _ = torch.max(features, 1)
